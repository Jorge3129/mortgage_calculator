...
const bankSlice = createSlice({
    ...
    reducers: {
        ...
        editBank: (state, {payload}: PayloadAction<NewBank>) => {
            let bank = findBank(state);
            if (bank) Object.assign(bank, payload)
            state.bankAction = undefined;
        },
        createBank: (state, {payload}: PayloadAction<NewBank>) => {
            //state.banks.push(new)
            state.bankAction = undefined;
        },
...
export const {setLoading, setBanks, setBankAction, deleteBank, editBank, createBank} = bankSlice.actions;
